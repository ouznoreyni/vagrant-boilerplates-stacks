...
setup_jfrog_home() {
    ...
    info 'export JFROG_HOME="/opt/jfrog"' | sudo tee -a /etc/environment
}
...
if [ "$os" = "ubuntu" ]; then
    info "Detected Ubuntu. Installing net-tools..."
    sudo apt-get install -y net-tools
    if ! command -v netstat &>/dev/null; then
        info "Failed to install net-tools. Please check the installation steps." >&2
        exit 1
    fi
fi
